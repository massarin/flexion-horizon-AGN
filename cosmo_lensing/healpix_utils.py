...
class HEALPixMap:
    ...
    def __init__(self, nside: int, data: np.ndarray, ordering: str = 'RING'):
        ...
        if data.shape[0] != self.npix:
            raise ValueError(f"Data size {data.shape[0]} does not match "
                           f"npix={self.npix} for nside={nside}")
    ...
    @classmethod
    def from_fits(cls, filename: str, field: int = 0, 
                  ordering: Optional[str] = None) -> 'HEALPixMap':
        """
        ...
        logger.info(f"Loaded HEALPix map from {filename}: "
                   f"nside={nside}, ordering={ordering}")
        
        return cls(nside, data, ordering=ordering)
    
    def to_fits(self, filename: str, coord: str = 'C', 
                column_name: str = 'SIGNAL', overwrite: bool = True):
        """
        ...
        hp.write_map(filename, self.data, nest=nest_flag,
                    coord=coord, column_names=[column_name], 
                    overwrite=overwrite, dtype=np.float64)
    ...
    def to_cartesian(self, center_ra: float, center_dec: float, 
                    radius_deg: float, npix: int) -> Tuple[np.ndarray, dict]:
        """
        ...
        logger.info(f"Projected {npix}×{npix} patch centered at "
                   f"(RA={center_ra:.2f}°, Dec={center_dec:.2f}°), "
                   f"radius={radius_deg:.2f}°")
        
        return cartesian_map, info
    
    def process_patches(self, patch_size_deg: float, overlap_deg: float,
        ...
        for i_dec in range(n_dec):
            ...
            for i_ra in range(n_ra):
                ...
                try:
                    ...
                    logger.warning(f"Failed to process patch at "
                                 f"(RA={ra_center:.1f}°, Dec={dec_center:.1f}°): {e}")
                    continue


def cartesian_to_healpix(ra: np.ndarray, dec: np.ndarray, 
                         nside: int, ordering: str = 'RING') -> np.ndarray:
    """
...
def healpix_to_cartesian(ipix: np.ndarray, nside: int,
                         ordering: str = 'RING') -> Tuple[np.ndarray, np.ndarray]:
    """
