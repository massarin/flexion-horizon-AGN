...
def generate_sis_deflection(
    ...
    """
    logger.info(
        f"Generating SIS deflection: σ_v={sigma_v:.3f}, "
...
def generate_nfw_deflection(
    ...
    alpha_magnitude = (
        inner_weight * mass / theta +  # Point mass-like
        outer_weight * np.sqrt(mass)  # SIS-like
    )
...
def generate_shear_field(
    ...
    logger.info(
        f"Shear field: γ₁={gamma1:.4f}, γ₂={gamma2:.4f}"
    )
...
def generate_convergence_field(
    ...
    """
    logger.info(
        f"Generating pure convergence field: κ={kappa:.3f}, "
        f"grid={grid_size}x{grid_size}"
    )
