...
class CosmologyCalculator:
    ...
    def __init__(
        self, H0: float = 67.66, Om0: float = 0.3111, Ob0: float = 0.0490, Tcmb0: float = 2.7255
    ):
        """
        ...
        """
        self.cosmo = FlatLambdaCDM(H0=H0 * u.km / u.s / u.Mpc, Om0=Om0, Ob0=Ob0, Tcmb0=Tcmb0 * u.K)
        self.h = H0 / 100.0

        logger.info(
            f"Initialized cosmology: H0={H0:.2f}, Om0={Om0:.4f}, " f"Ob0={Ob0:.4f}, h={self.h:.4f}"
        )

    def angular_diameter_distance(
        self, z1: Union[float, np.ndarray], z2: Union[float, np.ndarray] = 0.0
    ...
    def critical_density(self, z: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """
    ...
    def Omega_m(self, z: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
        """
    ...
    def sigma_crit(
        self, zl: Union[float, np.ndarray], zs: Union[float, np.ndarray]
    ) -> Union[float, np.ndarray]:
        """
        ...
        # Validate redshifts
        if np.any(zs <= zl):
            raise ValueError(f"Source redshift ({zs}) must be greater than lens redshift ({zl})")
